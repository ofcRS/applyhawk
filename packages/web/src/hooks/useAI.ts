...
export function useAI(settings: Settings) {
  ...
  const parseVacancy = useCallback(async (rawText: string): Promise<Vacancy> => {
    if (!client) {
    ...
    try {
      ...
      const result = await client.parseUniversalVacancy(
        rawText,
        (vars) => promptLoader.buildPromptFromTemplate('universal-vacancy-parse', vars, language)
      );
  ...
  const generatePersonalizedResume = useCallback(async (
    resume: Resume,
    vacancy: Vacancy
    ...
    try {
      ...
      const result = await client.generatePersonalizedResume(
        resume,
        vacancy,
        (vars) => promptLoader.buildPromptFromTemplate('resume-personalization', vars, language),
        null, // fitAssessment
      ...
      const message = err instanceof Error ? err.message : 'Failed to generate personalized resume';
      setError(message);
      throw err;
    } finally {
      setIsLoading(false);
  ...
  const generateCoverLetter = useCallback(async (
    ...
    try {
      ...
      const result = await client.generateCoverLetter(
        vacancy,
        baseResume,
        (vars) => promptLoader.buildPromptFromTemplate('cover-letter', vars, language),
        personalizedResume,
        null, // fitAssessment
        0.5,  // aggressiveness
        settings
